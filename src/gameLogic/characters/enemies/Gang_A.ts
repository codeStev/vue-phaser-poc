...
export default abstract class Gang_A extends Phaser.Physics.Arcade.Group{
    ...
    killCallback(gameObject: GameObjects.GameObject){
       (<Enemy> gameObject).body.world.on("worldbounds",function(body : Phaser.Physics.Arcade.Body, blockedUp : boolean, blockedDown : boolean, blockedLeft : boolean, blockedRight : boolean){
        if(blockedDown){
